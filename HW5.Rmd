...
```{r, warning=FALSE, message=FALSE}
length(unique(solea$salinity)) ##The range is from 2 to 34, so I will do four bins with width 8.
solea$proportion = with(solea, Solea_solea/length(Solea_solea)) 
##calculate proportional survivorship for each row (binned below during creation of plot)
...
##plot the raw data and add points for each "bin". 
##Y values are calculated by subseting proportional sums between each quartile
##against the midpoint of each bin.
points(6, sum(solea$proportion[solea$salinity <= 10 & solea$salinity >= 2]), 
       col = "hotpink4", pch = 19)
points(14, sum(solea$proportion[solea$salinity <= 18 & solea$salinity >= 10]), 
       col = "hotpink4", pch = 19)      
points(22, sum(solea$proportion[solea$salinity <= 26 & solea$salinity >= 18]), 
       col = "hotpink4", pch = 19)
points(30, sum(solea$proportion[solea$salinity <= 34 & solea$salinity >= 26]), 
       col = "hotpink4", pch = 19)
```
...
hs = read.csv("heat_shock_subset.csv")
